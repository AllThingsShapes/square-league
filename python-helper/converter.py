...
with open(file_path, 'r') as file, open(output_file_path, 'w') as output_file:
    for line in file:
        ...
        wld = split_line[8]
        wld_array = '[' + ', '.join(f'"{char}"' for char in wld) + ']'

        stat_str = '{ ' + f'id: {id}, color: \"{split_line[0]}\", name: \"{split_line[1]}\", wins: {split_line[2]}, losses: {split_line[3]}, draws: {split_line[4]}, power_ups: {split_line[5]}, damage_done: {split_line[6]}, wall_bounces: {split_line[7]}, last5: {wld_array}, image: \"{split_line[9].replace('\n', '')}\"' + ' },'
